...
static void  msm_serial_hs_rt_init(struct uart_port *uport)
{
	...
	/*
	 * -1, as vicos-oelinux does (0ebbfaa57).  CAF's 100 ms lets the port
	 * runtime-suspend between bursts; combined with the non-blocking
	 * pm_runtime_get() the VIC-4461 backport needs (section 9.8), a
	 * suspended port is not resumed in time and shows up as
	 * "runtime callback not invoked" followed by RX overruns.
	 */
