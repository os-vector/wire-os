...
/**
 *	flush_to_ldisc
 *	@port: tty port whose pending input should be delivered.
 *
 *	This routine is called by the TTY buffer worker to flush data from the
...
static void flush_to_ldisc_workqueue(struct work_struct *work)
{
...
int __init tty_buffer_init_kthread(void)
{
...
void tty_buffer_init(struct tty_port *port)
{
	...
	INIT_WORK(&buf->work, flush_to_ldisc_workqueue);
	kthread_init_work(&buf->kwork, flush_to_ldisc_kthread);
...
{
	return kthread_queue_work(&tty_buffer_worker, &port->buf.kwork);
}
...
bool tty_buffer_cancel_work(struct tty_port *port)
{
	...
	canceled = kthread_cancel_work_sync(&port->buf.kwork);
	return cancel_work_sync(&port->buf.work) || canceled;
}
