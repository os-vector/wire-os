...
# cleared every time
cleanList=(victor wired vic-cloud core-image-anki-initramfs rampost anki-version machine-robot-image system-conf extra-conf vic-engine vic-robot update-os update-engine wireutils initscript-anki rebooter vic-anim vic-switchboard vic-gateway-cert base-files libpvictor fake-hwclock purplpkg wireos-recovery)
