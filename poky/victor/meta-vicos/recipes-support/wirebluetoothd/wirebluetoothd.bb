...
SERVICE_FILE = "wirebluetoothd.service"
...
EXTERNALSRC = "${WORKSPACE}/anki/wirebluetoothd"

do_clean:append () {
    dir = bb.data.expand("${EXTERNALSRC}", d)
    os.system('cd "%s" && rm build/wirebluetoothd' % dir)
}
...
run_victor() {
  ...
  env \
    ...
    -u userfsdatadir \
    -i PATH=/usr/bin:/bin:/usr/sbin:/sbin HOME=$HOME PWD="${WORKSPACE}/anki/wirebluetoothd" \
    "$@"
}
...
do_install () {
    install -d ${D}/usr/bin
    install -p -m 0755 ${WORKSPACE}/anki/wirebluetoothd/build/wirebluetoothd ${D}/usr/bin/
}

FILES:${PN} += "usr/bin/wirebluetoothd"
